...
void test_tokenizer_word_tokens_only(void) {
    ...
    TEST_ASSERT_TRUE(token_list2->type == TOK_NULL_TYPE); 
...
void test_tokenizer_ampersand_only(void) {
    token_t *token_list;
 
    token_list = tokenizer("&");
    TEST_ASSERT_TRUE(token_list[0].type == TOK_AMPS_TYPE);
    TEST_ASSERT_TRUE(token_list[1].type == TOK_NULL_TYPE);

    token_list = tokenizer("&&&&");
    TEST_ASSERT_TRUE(token_list[0].type == TOK_AMPS_TYPE);
    TEST_ASSERT_TRUE(token_list[1].type == TOK_AMPS_TYPE);
    TEST_ASSERT_TRUE(token_list[2].type == TOK_AMPS_TYPE);
    TEST_ASSERT_TRUE(token_list[3].type == TOK_AMPS_TYPE);
    TEST_ASSERT_TRUE(token_list[4].type == TOK_NULL_TYPE);
}
...
int main(void) {
    ...
    RUN_TEST(test_tokenizer_ampersand_only);
