...
- `char **tokens`: This array of char pointers represents the command and its parameters. It is generated by simply copying the `char *str` reference of the `WORD` tokens. At the last index of the array a `NULL` pointer is placed. This is done because the field

Additionally this exactly the variable type the
